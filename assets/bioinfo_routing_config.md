...
## 十六、司空工作室 R 模板库（2026-09-06 集成，即开即用）

> R 代码模板大全：每套 = R 脚本 + 示例数据 + PDF/PNG 成图参照。路由方式：用户报图型 → `assets/SIKONG_INDEX.md` 检索 → 套用 R 脚本。

| 目录 | 内容 | 规模 |
|------|------|------|
| `assets/sikong_50/` | 50 套基础图型（柱状/箱线/小提琴/气泡/热图/火山/GSEA/森林/生存/ROC…） | 690 文件 |
| `assets/sikong_360/` | 361 个进阶模板：293 R + 112 Rmd（降维 PCoA/OPLS-DA/RDA/CCA、单细胞 UMAP/plot1cell/muscat、环形/组合/注释热图、网络图、CNV、进化树、circos、WGCNA、ROC、三元图、3D 基因组、免疫浸润、生存亚组…） | 4078 文件 |
| `assets/SIKONG_INDEX.md` | 全量索引：361 目录逐一列出 + 脚本文件名 | — |
| `docs/文献读图知识攻略.pdf` | 配套读图教程 | — |

**快速检索高频模板**：
- 瀑布图 → `【100】`；曼哈顿图 → `【102】`；环形热图 → `【104】【116】【173】`
- PCoA + 侧边箱线 → `【127】`；OPLS-DA/PLS-DA → `【125-126】`；envfit/RDA/CCA → `【128】`
- 复现 Nature UMAP → `【229】`；plot1cell 单细胞聚类 → `【219】`；差异基因 muscat → `【224】`
- 生存曲线 → `【240】`；最佳分组生存 → `【242】`；timeROC → `【333】`；免疫治疗预测 → `【353】`
- 进化树家族 → `【18】【190】`；基地circos → `【250】`；3D 基因组 → `【239】`；三元图 → `【243】`
- WGCNA → `【351】`；GO 网络图 → `【115】`；Ternary → `【243】`；多序列比对 → `【198】`

**与路由函数协同**：`bioinfo_router.py` 找不到的稀有图型或 R 专区模板 → 回退到本表检索即可（属"R 工具链 21 种"注册表之外的人工索引层）。

**过滤说明**：mp4 教程视频（1.13GB）、.RData 工作区快照（2.98GB）、>20MB 大文件、.Rhistory/.DS_Store 未入库。原始 zip 保留在 `E:\sci_figure_master\`，需要被过滤掉的媒体/大数据可随时补下。
