...
为了提取瀑布图中的sample，修改了oncoprint函数，保存为`oncoPrint_plus.R`，搜索#Ya#查看修改的位置。

**注意：**在后面所有画图命令中，column_order和remove_empty_columns这两个参数，都要跟p1的参数一致。

```{r,fig.width=10,highlight=4}
library(ComplexHeatmap)
source("oncoPrint_plus.R") #该文件位于当前文件夹

#提取瀑布图中画出的sample
p1 <- oncoPrint_plus(mygene[1:(ncol(mygene)-1)], get_type = function(x) x,
...
sampleOrder <- data.frame(p1$sampleOrder)
sampleOrder$oriOrder <- row.names(sampleOrder)
sampleOrder <- sampleOrder[order(as.numeric(sampleOrder[,2]),decreasing=F),]
...
p <- oncoPrint(mygene[1:(ncol(mygene)-1)], get_type = function(x) x,
               ...
               show_row_barplot = FALSE, #不画百分比的bar plot
               bottom_annotation = my_annotation,#把临床信息画在下面
               split = mygene$path, #按照pathway分开画
...
```{r}
library(ComplexHeatmap)
source("oncoPrint_plus.R") #该文件在当前文件夹
