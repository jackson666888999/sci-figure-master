...
- 使用TCGA数据的小伙伴可以直接使用压缩包里的`eff_length.csv`文件，就可以跳过这步，直接从“read count转FPKM”开始运行。
...
这里调用`get_eff_len.R`文件里的函数实现从下载GTF文件到计算基因长度，计算的是每个基因的最长转录本(外显子之和)。该函数作者：徐州更，出自`FigureYa23 count2TPM`

```r
source("get_eff_len.R") #get_eff_len.R文件位于当前文件夹

#url <- "ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_28/gencode.v28.annotation.gtf.gz"
file<- "gencode.v22.annotation.gtf" #你下载的gtf文件的文件名
eff_length <- get_eff_len(file = file, #或者换成file = url，将会自动下载解压缩
...
以第一个sample为例，raw count 和 count转成的FPKM的相关系数是0.92；

raw count 和 从TCGA直接下载的FPKM的相关系数是 0.89。

而count转成的FPKM和从TCGA下载的FPKM的相关系数是0.98, 证明了count2FPKM的代码是有效的。
```
