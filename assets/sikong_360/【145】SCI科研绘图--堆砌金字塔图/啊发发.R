...
p <- df %>% 
  ggplot(aes(y=age,x = ifelse(sex=="Log Koc",0,val),fill =cps)) + 
  geom_bar(stat = "identity")+
  scale_fill_manual(values=met.brewer("Hiroshige",9))+
  scale_x_continuous(guide = "prism_minor",expand = c(0,0),limits=c(0,30),
                     breaks=seq(-15,15,2))+
  ...
  annotate(geom="text",y=16,x=0.5,label="Log Koc SCCPs",size=4,fontface="bold")+
  annotate(geom="text",y=16,x=-0.5,label="Log Koc MCCPs",size=4,fontface="bold")
