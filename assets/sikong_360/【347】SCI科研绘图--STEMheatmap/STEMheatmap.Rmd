...
data_nest <- data_nest_2 %>% 
  mutate(., data_2 = map(.$data, ~clust_order(.x, row_name = "GeneSymbol"))) %>% 
  select(-data) %>% rename(data = data_2) %>% 
  rbind(data_nest_1) 
...
```{r}
# 提出基因名称
data_nest %<>% mutate(.,Gene = map(.$data, ~.x$GeneSymbol))
# 累加
...
data_nest %<>% mutate(., tile_data = map2(.$data, .$range_y, 
...
data_nest %<>% mutate(., right_coord = map(.$range_y, link_right))
```
...
left_coord %<>% mutate(., coord_left = map(.$data, trans_coord)) %>% 
  select(-data) 
```
...
link_coord <- data_nest %>% select(Profile, right_coord, n) %>% 
  ...
  mutate(., coords = map2(.$coord_left, .$right_coord, rbind)) %>% 
  mutate(., coords_2 = map(.$coords, ~arrange(.x, order))) %>% 
  select(Profile, coords = coords_2, n) 
...
line_data <- profiletable_extract %>% group_by(Profile) %>% 
  ...
  mutate(., plot_ls = map(.$data, line_generate)) %>% 
  left_join(left_coord, by = "Profile")
...
width_line <- 10
line_data %<>% 
  mutate(., rectangle_range = map(.$coord_left, 
                                  ~line_coord(.x, width_line = width_line))) %>% 
  select(Profile, plot_ls, rectangle_range)
```
...
text_data <- left_coord %>% 
  mutate(., y = map(.$coord_left, ~.x$y)) %>%  
  mutate(., y_up = map(.$y, ~.x[1]), y_down = map(.$y, ~.x[2])) %>% 
  select(Profile, y_up, y_down) %>% unnest() %>% arrange(Profile) %>% 
