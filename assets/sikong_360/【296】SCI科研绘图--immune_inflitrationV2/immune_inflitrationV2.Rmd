...
![](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5749656/bin/koni-07-02-1382792-g004.jpg)
...
targetnum <- which(tcgasample %in% targetCancerCase$sample) #V2#上一版的%in%前后写反了
...
targetCancerTPM <- read.delim("targetCancerTPM", row.names = 1)
targetCancerTPM[1:3, 1:3]
dim(targetCancerTPM)

#有TPM值的目标癌症的sample ID，后面会用到
targetSample <- str_replace_all(colnames(targetCancerTPM),"[.]","-")
```
...
```{r}
targetCancerTPMestimates <- read.csv("MCPcounter_output.csv", row.names = 1)
...
row.names(annotation_col) <- colnames(targetCancerTPMestimates)
#此处使用默认配色，如果想自己设置临床信息的配色或更改更多参数，可参考FigureYa17或FigureYa9heatmap
