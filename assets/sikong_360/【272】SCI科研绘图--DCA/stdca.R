stdca <- function(data, outcome, ttoutcome, timepoint, predictors, xstart=0.01, xstop=0.99, xby=0.01, 
  ...
  if (graph==TRUE) {
    ...
    if(intervention==TRUE) {
      ...
      for(m in 1:pred.n) {
        if (smooth==TRUE){
          lines(interv$threshold,data.matrix(interv[paste(predictors[m],"_sm",sep="")]),col=m,lty=1,lwd=3)     
        } else {
          lines(interv$threshold,data.matrix(interv[predictors[m]]),col=m,lty=1,lwd=3)
        }
