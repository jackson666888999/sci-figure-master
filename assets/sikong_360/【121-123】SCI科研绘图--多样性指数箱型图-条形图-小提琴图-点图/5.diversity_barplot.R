...
stat.test$p.adj.signif<-cut(stat.test$p,breaks =c(0,0.001,0.01,0.05,1),
                            labels = c('***','**','*','ns')  )
ggbarplot(df2, x = "group", y = "new", fill = "group",
          add = c('mean_se'),
          add.params = list(size=0.5,width=0.4))+ 
...
write.table(stat.test2,paste0('step5.',myindex,'.bar.pdf.stat.xls'),row.names = F,sep = '\t',quote = F)

ggsave(paste0('step5.',myindex,'.bar.pdf'),width = 4,height = 4)
